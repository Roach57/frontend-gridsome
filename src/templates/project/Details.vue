<template>
    <Layout>
        <el-card shadow="never" style="min-height: 400px">
            <div slot="header">
                <el-row>
                    <el-col :span="12">
                        <span>{{$page.post.name}}</span>
                    </el-col>
                    <el-col :span="12">
                        <div style="text-align: right;">
                            <el-button style="padding: 3px 0" type="text" icon="el-icon-share">分享</el-button>
                            <el-button style="padding: 3px 0" type="text" icon="el-icon-back">前往GitHub</el-button>
                            <el-button style="padding: 3px 0" type="text" icon="el-icon-more-outline">更多项目</el-button>
                        </div>
                    </el-col>
                </el-row>
            </div>
            <div style="font-size: 0.9rem;line-height: 1.5;color: #606c71;">
                发布 {{$page.post.created_at | date('YYYY-MM-DD HH:mm:ss')}}
                <br> 更新 {{$page.post.updated_at | date('YYYY-MM-DD HH:mm:ss')}}
            </div>
            <div style="font-size: 1.1rem;line-height: 1.5;color: #303133;padding: 20px 0px 0px 0px">
                {{$page.post.description}}
            </div>
            <div style="font-size: 1.1rem;color: #303133;padding: 15px 0px 15px 0px;border-bottom: 1px solid #E4E7ED;">
                <el-row>
                    <el-col :span="16"  style="padding-top: 5px">
                        <el-tooltip effect="dark" :content="'star '+$page.post.stargazers_count" placement="bottom">
                            <i class="el-icon-star-off" style="margin: 0px 5px 0px 0px"></i>
                        </el-tooltip>
                        {{$page.post.stargazers_count}}
                        <el-tooltip effect="dark" :content="'watch '+$page.post.watchers_count" placement="bottom">
                            <i class="el-icon-view" style="margin: 0px 5px 0px 15px"></i>
                        </el-tooltip>
                        {{$page.post.watchers_count}}
                        <el-tooltip effect="dark" :content="'fork '+$page.post.forks_count" placement="bottom">
                            <i class="el-icon-bell" style="margin: 0px 5px 0px 15px"></i>
                        </el-tooltip>
                        {{$page.post.forks_count}}
                    </el-col>
                    <el-col :span="8" style="text-align: right">

                        <el-tag size="small" type="success">{{$page.post.language}}</el-tag>
                    </el-col>

                </el-row>
            </div>
            <div v-html="mdToHtml($page.post.content)" v-if="$page.post.content" class="markdown-body" style="padding-top: 20px"></div>
            <div v-if="!$page.post.content" style="padding: 20px 0px 20px 0px;text-align: center">
                <font style="font-size: 30px;color:#dddddd ">
                    <b>还没有介绍 (╯°Д°)╯︵ ┻━┻</b>
                </font>
            </div>
        </el-card>
    </Layout>
</template>

<page-query>
query ($id: ID){
    post: strapiProjects (id: $id) {
        id
        name
        html_url
        stargazers_count
        watchers_count
        forks_count
        language
        description
        content
        created_at
        updated_at
    }
}

</page-query>

<script>
import MarkdownIt from 'markdown-it'
const md = new MarkdownIt()


export default {
    data() {
        return {
            project: {
                name: ""
            },
            loading: false,
        }
    },
    methods: {
        mdToHtml (content) {
            return md.render(content)
        }
    }
}
</script>